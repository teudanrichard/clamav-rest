...
Changes reach `main` through pull requests with the `quality` and `integration` checks passing. CODEOWNERS identifies the personal maintainer account for review; security-sensitive changes to authentication, image publication, dependencies, and deployment defaults need explicit maintainer review.
